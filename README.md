...
- Customer-only: customers order through the bot. Staff operations live in the
  website dashboard.
...
sends it to the kitchen. Do not change `WAYL_ENV` to `live` before a separate
...
- **Orders**: a cart is an `orders` row with `status = 'pending'`; confirming it
  mints a unique `ORD-XXXXX` code and shows it to the customer.
...
`test/support/fake-telegram-api.js` (local Bot API), so customer flows are
