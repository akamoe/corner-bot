  ...
  return slots
    .map((slot) => {
      ...
      return {
        ...
        spots_left: Math.max(0, Number(slot.max_orders || 0) - current)
      }
    })
    .filter((slot) => !isSlotTimePast(slot.slot_time, date) && slot.spots_left > 0)
}
...
export async function getSlotBookability(slotId, date = new Date()) {
  ...
  if (current >= max) return { ok: false, reason: 'full', slot }

  return { ok: true, slot, spotsLeft: max - current }
}
