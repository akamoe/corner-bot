...
import { setupStudentMenu } from './handlers/student-menu.js'
...
export function createBot(token = process.env.BOT_TOKEN, options = undefined) {
  ...
  bot.command('status', async (ctx) => {
    ...
    return safeReply(ctx, studentOrderCard(order), {
  ...
  bot.command('menu', async (ctx) => {
    await ctx.reply('استخدم زر *🍽 تصفح المنيو* 👇', {
      parse_mode: 'Markdown',
      ...studentKeyboard()
    })
  })
...
function helpText() {
  return (
    ...
    '🛒 *سلتي* — عدل الكميات وأكد الطلب\n' +
    '📦 *طلباتي* — تابع حالة طلبك أو الغيه\n' +
    '🕐 *وقت الاستلام* — تختاره وقت تأكيد الطلب\n\n' +
    'بعد التأكيد راح يطلعلك *رمز* (مثل ORD-7KQ2M). وريه عند الكاونتر.\n' +
