...
export function setupStudentCart(bot) {
  ...
  bot.action(/^cart_rm_(.+)$/, async (ctx) => {
    ...
    try {
      ...
      console.error('[studentCart] cart_rm_ error, orderItem:', orderItemId, err.message, err)
  ...
  bot.action('clear_cart', async (ctx) => {
    ...
    try {
      ...
      console.error('[studentCart] clear_cart error userId:', user.id, err.message, err)
      await ctx.answerCbQuery('❌ فشل تفريغ السلة.')
    }
  })
  ...
  bot.action(/^slot_(.+)$/, async (ctx) => {
    ...
    if (slotError) {
      console.error('[studentCart] Error fetching slot after confirm, slotId:', slotId, 'cartId:', cart.id, slotError.message, slotError)
    }
