...
import { notifyCashiers, queueCashierNotices } from '../notifications.js'
...
export function setupStudentCart(bot) {
  ...
  bot.action(/^pay_cash_([0-9a-f-]{36})$/, async (ctx) => {
    ...
    let order
    try {
      await queueCashierNotices(user.id, cart.id)
      ...
      await ctx.reply('⚠️ ما قدرنا نثبت الطلب أو نجهز إشعار الكاونتر. سلتك محفوظة، جرب مرة ثانية.')
      return
    }
    ...
    if (notice.failed || (notice.pending && !notice.sent)) {
      await safeReply(ctx,
