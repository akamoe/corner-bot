...
export async function showCart(ctx, userId, telegramId) {
  ...
  const summary = await safeRespond(ctx, summaryText(totals, cart.notes), {
    ...summaryKeyboard()
  })
...
async function offerSlots(ctx, userId, telegramId) {
  ...
  const rows = slots.map((slot) => [
    Markup.button.callback(`🕐 ${slot.label} — باقي ${slot.spots_left}`, `slot_${slot.id}`)
  ])
...
export function setupStudentCart(bot) {
  ...
  bot.action(/^cart_qty_up_(.+)$/, async (ctx) => {
    ...
    await updateCartItemQuantity(itemId, 1).catch((err) =>
      console.error('[studentCart] qty_up failed itemId:', itemId, err.message, err)
    )
    await ctx.answerCbQuery('➕').catch(() => {})
  ...
  bot.action(/^cart_qty_down_(.+)$/, async (ctx) => {
    const user = await getOrCreateUser(ctx.from.id)
    const itemId = ctx.match[1]
    const qty = await updateCartItemQuantity(itemId, -1).catch((err) => {
      console.error('[studentCart] qty_down failed itemId:', itemId, err.message, err)
      return undefined
    })
  ...
  bot.action('cart_note', async (ctx) => {
    ...
    await safeReply(
      ctx,
      '📝 اكتب ملاحظتك (مثلاً: بدون بصل، حار زيادة) وابعثها.\nاذا ما تريد ملاحظة تجاهل هاي الرسالة.',
      { ...Markup.inlineKeyboard([[Markup.button.callback('↩️ رجوع للسلة', 'view_cart')]]) }
    )
  })
  ...
  bot.action(/^slot_([0-9a-f-]{36})$/, async (ctx) => {
    ...
    await notifyCashiers(bot, full.data || order)
  })
}
