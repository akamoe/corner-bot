...
export async function showCategoryItems(ctx, categoryId, page = 0) {
  ...
  if (pages > 1) {
    const nav = []
    if (current > 0) nav.push(Markup.button.callback('السابق ⬅️', `catp_${categoryId}_${current - 1}`))
    nav.push(Markup.button.callback(`${current + 1}/${pages}`, 'noop'))
    if (current < pages - 1) nav.push(Markup.button.callback('➡️ التالي', `catp_${categoryId}_${current + 1}`))
    rows.push(nav)
  }
...
export function setupStudentMenu(bot) {
  ...
  bot.action('confirm_item', async (ctx) => {
    ...
    const currentItem = await getMenuItem(state.itemId)
    if (!currentItem || currentItem.is_available === false) {
      await orderFlowState.delete(userId)
      await ctx.answerCbQuery('هاي الوجبة مو متاحة هسة.', { show_alert: true }).catch(() => {})
      return showCategories(ctx)
    }
    const currentGroups = await getItemToppingGroups(state.itemId)
    const currentIds = new Set(currentGroups.flatMap((group) => (group.toppings || []).map((t) => t.id)))
    const selectedNow = state.selectedToppings.filter((id) => currentIds.has(id))
    const oldPrice = calculateFinalPrice(state.basePrice, state.selectedToppings, state.groups)
    const newPrice = calculateFinalPrice(currentItem.price, selectedNow, currentGroups)
    if (selectedNow.length !== state.selectedToppings.length || newPrice !== oldPrice
      || !validateRequiredGroups(currentGroups, selectedNow)) {
      await orderFlowState.set(userId, {
        ...state, itemName: currentItem.name, basePrice: Number(currentItem.price),
        groups: currentGroups, selectedToppings: selectedNow
      })
      await ctx.answerCbQuery('تغيرت الوجبة أو الإضافات. راجع السعر والاختيارات.', { show_alert: true }).catch(() => {})
      return sendCustomizationMessage(ctx, userId)
    }
    ...
    const { itemId, quantity } = state
    const itemName = currentItem.name
    const selectedToppings = selectedNow
  ...
  bot.action('cancel_customize', async (ctx) => {
    ...
    await safeEdit(ctx, '❌ لغينا التخصيص.', {
      ...Markup.inlineKeyboard([[Markup.button.callback('↩️ رجوع للفئة',
        state?.categoryId ? `cat_${state.categoryId}` : 'catroot')]])
    })
  })
