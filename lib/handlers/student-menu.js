...
async function sendCustomizationMessage(ctx, userId) {
  ...
  try {
    ...
    console.error('[sendCustomizationMessage] Error:', err.message, err)
    if (err.message.includes('not modified')) {
      return
    }
    await ctx.reply(text.replace(/[*_`\\[\\]]/g, ''), {
      ...Markup.inlineKeyboard(keyboard)
    }).catch(e => console.error('[sendCustomizationMessage] Fallback reply failed:', e.message))
  }
}
...
export function setupStudentMenu(bot) {
  ...
  bot.action(/^item_(.+)$/, async (ctx) => {
    try {
      ...
      console.error('[studentMenu] item_ action error userId:', ctx.from?.id, 'itemId:', ctx.match?.[1], err)
