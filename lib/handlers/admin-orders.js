...
import { daysAgoIso, todayIso, dayRange } from '../time.js'
import { formatIQD } from '../money.js'
import { formatOrderItems, safeReply } from './helpers.js'
...
async function showAnalytics(ctx, days) {
  ...
  const topItems = [...itemCounts.entries()]
    ...
    .slice(0, 5)
    .map(([name, v], idx) => `${idx + 1}. ${name} — ${v.qty} sold (${formatIQD(v.revenue)})`)
    .join('\n') || '(no items sold)'
  ...
  await ctx.reply(
    ...
    `💵 Revenue: *${formatIQD(revenue)}*\n` +
    `🧾 Avg order: *${formatIQD(avgOrder)}*\n` +
