 ...
 * Uses upsert with jsonb_set-style merge on the server side to
 * avoid the read-before-write race condition.
 */
async function setState(userId, key, value) {
  ...
  if (error) console.error('[setState] userId:', userId, 'key:', key, error.message, error)
}
