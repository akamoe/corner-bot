...
import { cashierOrderCard, orderStatusButtons, studentOrderCard } from './handlers/helpers.js'
...
async function notifyPaid(payment) {
  ...
  if (payment.environment !== 'live' || !order || order.status !== 'confirmed') return
  const staffClaim = await claimNotification(payment, 'staff')
  if (!staffClaim) return
  try {
    const { data: staff, error } = await supabase.from('staff')
      .select('telegram_id').eq('is_active', true).in('role', ['cashier', 'admin'])
    if (error) throw error
    for (const member of staff || []) {
      if (!member.telegram_id) continue
      await bot.telegram.sendMessage(member.telegram_id,
        `🔔 *طلب جديد ومدفوع!*\n\n${cashierOrderCard(order)}`,
        { parse_mode: 'Markdown', reply_markup: { inline_keyboard: orderStatusButtons(order) } })
    }
    await finishNotification(payment, 'staff', true)
  } catch {
    await finishNotification(payment, 'staff', false)
    console.error('[wayl] staff notification failed')
  }
}
...
export async function reconcileExpiredPayments(limit = 20) {
  ...
  for (const kind of ['customer', 'staff']) {
    ...
    const { error: releaseError } = await supabase.from('telegram_wayl_payments')
      .update({ [claimed]: null }).eq('status', 'paid')
      .eq('environment', config.environment).is(notified, null).lt(claimed, stale)
    if (releaseError) throw new Error('NOTIFICATION_RETRY_RESET_FAILED')
  }
  const retryQuery = supabase.from('telegram_wayl_payments')
    .select('*').eq('status', 'paid').eq('environment', config.environment)
  const { data: unnotified, error: retryError } = await (config.environment === 'test'
    ? retryQuery.is('customer_notified_at', null)
    : retryQuery.or('customer_notified_at.is.null,staff_notified_at.is.null'))
